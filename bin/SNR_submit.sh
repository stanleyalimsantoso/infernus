...
for i in $(seq $SLURM_ARRAY_TASK_ID 2048 $total_jobs); do
	...
	python ${INFERNUS_DIR}/bin/SNR_compute.py --jobindex=$i --totalworkers=$n_workers --argsfile=$jsonfile $streamline \
	...
	if [ "$injfile" != "None" ]; then
		...
		python ${INFERNUS_DIR}/bin/serve_hybrid.py --jobindex=$i --argsfile=$jsonfile
		pid=$!
		wait $pid
	fi
